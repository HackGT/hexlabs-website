...
WORKDIR /usr/src/hexlabs-website
...
WORKDIR /usr/src/hexlabs-website
COPY . .
COPY --from=deps /usr/src/hexlabs-website/node_modules ./node_modules
RUN yarn build
...
COPY --from=builder /usr/src/hexlabs-website/package.json ./package.json

RUN addgroup -g 1001 -S nodejs
RUN adduser -S nextjs -u 1001
RUN chown -R nextjs:nodejs /usr/src/hexlabs-website/.next
USER nextjs
